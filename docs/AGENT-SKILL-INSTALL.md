...
| `_shared` | Support files (not a standalone skill) | **Yes** whenever Word or PowerPoint is installed |
...
for name in excalidraw-diagram word-document powerpoint-presentation _shared; do
...
**Do not** copy the whole `architect-doc-skill` repo into `~/.cursor/skills/` — only the four folders inside `skills/`.
...
ls -1 "$CURSOR" | sort
# Expect: _shared  excalidraw-diagram  powerpoint-presentation  word-document
...
Re-run Step 2 (legacy cleanup if needed) and Step 3 or 4 (copy all four folders again). Then ask them to open a new agent chat.
