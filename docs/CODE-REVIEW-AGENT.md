...
11. For commit/diff/PR/branch reviews, also **append** (never full-rewrite) findings to `/tmp/<topic>-handoff.md` per the `review-handoff` skill (Cursor `~/.cursor/skills/review-handoff/SKILL.md`, Copilot `~/.copilot/skills/review-handoff/SKILL.md`); source tree stays read-only.
