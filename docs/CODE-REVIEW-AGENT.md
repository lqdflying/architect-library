...
11. For commit/diff/PR/branch reviews, also **append** (never full-rewrite) findings to `/tmp/<topic>-handoff.md` per the review-handoff protocol (Cursor `~/.cursor/rules/review-handoff-reconciliation.mdc`, Copilot `~/.copilot/copilot-instructions.md`); source tree stays read-only.
