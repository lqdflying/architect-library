...
Use the **Context7 MCP server** to fetch current, accurate documentation for any library, framework, SDK, API, or CLI tool. Training data may be outdated — always verify against live docs.

> **MCP server name**: The server is typically configured as `context7` in `.vscode/mcp.json` or VS Code user settings. If tool calls fail with "server not found", check the actual server name in your MCP configuration and use that name instead.
...
Otherwise, call `resolve-library-id` on the Context7 MCP server:
...
2. Fall back to **web search** for the library's official docs
3. For Azure/Microsoft libraries, use **Microsoft Learn MCP** instead
