...
# Notion MCP Operations (Cursor)

Use the **Notion MCP server** (`plugin-notion-workspace-notion`) for all Notion workspace operations. Training-data assumptions about Notion APIs are unreliable — follow this skill and live tool schemas.

> **MCP server name**: In Cursor the Notion plugin registers as `plugin-notion-workspace-notion`. If tool calls fail with "server not found", check Settings → MCP for the actual server name.
...
| `notion://docs/enhanced-markdown-spec` | Before writing unfamiliar Markdown — fetch via `notion-fetch` or `FetchMcpResource` |
...
Always discover schemas with `GetMcpTools` before calling tools with unfamiliar arguments.
...
```
GetMcpTools({ server: "plugin-notion-workspace-notion" })
...
CallMcpTool({
  server: "plugin-notion-workspace-notion",
  toolName: "notion-fetch",
  arguments: { id: "notion://docs/enhanced-markdown-spec" }
})
```

Or `FetchMcpResource` if your client exposes the `notion://docs/enhanced-markdown-spec` resource.
