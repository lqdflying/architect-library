...
For every finding that survived Phase 3 validation, produce a structured JSON object conforming to the schema defined in `report-schema.json` (in the same `references/` directory as this file — read it via the Read tool before writing output). Write the result to `<output-dir>/findings.json`.
...
1. Read `report-schema.json` from this skill's `references/` directory. Follow it exactly — `additionalProperties: false` is enforced, so extra fields will make the output invalid.
